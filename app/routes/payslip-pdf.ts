...
export async function loader({ request, params, context }: Route.LoaderArgs) {
	...
	const row = await env.DB.prepare(`SELECT p.employee_id employeeId,e.full_name fullName,e.employee_code employeeCode,COALESCE(e.ic_number, '—') icNumber,COALESCE(e.epf_number, '—') epfNumber,COALESCE(e.bank_name, 'Maybank') bankName,COALESCE(e.bank_account_number, '—') bankAccountNumber,r.period,r.pay_date payDate,pr.gross_pay_sen grossPaySen,pr.total_deductions_sen totalDeductionsSen,pr.net_pay_sen netPaySen,pr.breakdown_json breakdownJson FROM payslips p JOIN employees e ON e.id=p.employee_id JOIN payroll_runs r ON r.id=p.payroll_run_id JOIN payroll_results pr ON pr.id=p.payroll_result_id WHERE p.id=?`).bind(params.id).first<{ employeeId: string; fullName: string; employeeCode: string; icNumber: string; epfNumber: string; bankName: string; bankAccountNumber: string; period: string; payDate: string; grossPaySen: number; totalDeductionsSen: number; netPaySen: number; breakdownJson: string }>();
