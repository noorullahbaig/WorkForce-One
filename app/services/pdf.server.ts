...
export async function payslipPdf(row:{fullName:string;employeeCode:string;period:string;payDate:string;grossPaySen:number;totalDeductionsSen:number;netPaySen:number;breakdownJson:string}) {
	...
	addHeader(page,bold,regular,`Payslip · ${row.period}`,"Merdeka Coffee Sdn. Bhd. · 202001028884");page.drawText(row.fullName,{x:42,y:720,size:16,font:bold});page.drawText(`${row.employeeCode} · Pay date ${row.payDate}`,{x:42,y:703,size:9,font:regular,color:rgb(.35,.42,.4)});page.drawRectangle({x:42,y:610,width:511,height:65,color:rgb(.91,.96,.94)});page.drawText("NET PAY",{x:60,y:650,size:8,font:bold,color:rgb(.03,.43,.31)});page.drawText(money(row.netPaySen),{x:60,y:622,size:24,font:bold,color:rgb(.04,.12,.16)});
	page.drawText("EARNINGS",{x:42,y:570,size:8,font:bold});page.drawText("DEDUCTIONS",{x:315,y:570,size:8,font:bold});const earning=[["Base pay",b.basePaySen],["Overtime",b.overtimePaySen],["Allowances",b.allowanceSen],["Bonus",b.bonusSen],["Gross pay",row.grossPaySen]] as const;const deductions=[["EPF",b.epfEmployeeSen],["SOCSO",b.socsoEmployeeSen],["EIS",b.eisEmployeeSen],["PCB",b.pcbSen],["Total deductions",row.totalDeductionsSen]] as const;earning.forEach(([label,value],i)=>{page.drawText(label,{x:42,y:540-i*28,size:9,font:i===4?bold:regular});page.drawText(money(value),{x:205,y:540-i*28,size:9,font:i===4?bold:regular})});deductions.forEach(([label,value],i)=>{page.drawText(label,{x:315,y:540-i*28,size:9,font:i===4?bold:regular});page.drawText(money(value),{x:465,y:540-i*28,size:9,font:i===4?bold:regular})});page.drawText("Confidential payslip · Finalised payroll record",{x:42,y:35,size:7,font:regular,color:rgb(.4,.45,.43)});return pdf.save();
}
