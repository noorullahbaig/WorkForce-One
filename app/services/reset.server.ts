export async function resetDemoData(db: D1Database) {
	...
	await db.batch([
		...
		db.prepare("UPDATE leave_requests SET status=CASE id WHEN 'lr-001' THEN 'pending' ELSE 'approved' END,reviewed_by=CASE id WHEN 'lr-001' THEN NULL ELSE 'user-admin' END,reviewed_at=CASE id WHEN 'lr-001' THEN NULL ELSE reviewed_at END,updated_at=?").bind(now),
		db.prepare("UPDATE leave_balances SET used_days=CASE WHEN employee_id='emp-007' AND leave_type_id='leave-annual' THEN 2 WHEN employee_id='emp-008' AND leave_type_id='leave-unpaid' THEN 1 ELSE 0 END"),
